...
- ✅ **Phase 9** — Notifications UI, global search, @mentions, market resolution, bookmarks page, onboarding wired to Supabase.
  - **Notifications.** New `lib/db/notifications.ts` (`listMyNotifications`, `countMyUnread`, `markAllRead`, `markOneRead`, `fanoutNotifications`). Generic payload shape covers follow/comment/reply/like/mention/trade/market_resolved/boost_ended. `app/notifications/actions.ts` exposes `markAllReadAction` / `markOneReadAction` with revalidation. New shared `components/notifications-list.tsx` renders both the `/notifications` page and the popover variant — per-type icons + colors, unread dot, mark-one-on-click navigation, optimistic mark-all-read, sonner toasts. `components/notification-bell.tsx` rewritten: pulls real rows from Supabase on mount, subscribes to `useNotificationsChannel`, shows a count badge (`9+` clamp), and renders `NotificationsList` inline in a Popover. Falls back cleanly when there's no Supabase env (shows mock entries on the page, empty popover).
  - **Global search.** New `lib/db/search.ts` powered by the trigram indexes from `0001_init.sql` — searches profiles, markets, and posts with capped per-bucket result counts (8 each), plus `searchProfilesByPrefix` for the mention popover. `/api/search?q=…` returns `{profiles, markets, posts}`; `/api/mentions?q=…` returns username prefixes. A `lib/mock-search.ts` fallback mirrors the shape so the dev demo without Supabase still gets results. `components/global-search.tsx` is a Cmd-palette-style modal: debounced fetch (180ms), ↑/↓ navigation, Enter to open, grouped by People/Markets/Posts, footer kbd hints. The topbar binds Cmd/Ctrl-K to open it and removes the old placeholder dialog.
  - **@mentions.** New `lib/mentions.ts` exports `extractMentions(text)` and `linkifyMentions(html)`. `RichEditor` gains `insertText` and `replaceMention` imperative methods. `components/post/mention-popover.tsx` watches a partial token after `@` and fetches `/api/mentions`, keyboard-navigable with Tab/Enter/Esc. The composer hooks editor `onChange` to detect a trailing `@xxx` and renders the popover below the editor; selection calls `replaceMention(username)` to swap the partial. On publish, the composer fires `notifyMentionsAction({usernames, post_id, excerpt})` (in `src/app/feed/actions.ts`) which resolves usernames → profile ids and fans out `mention` notifications via the service-role client (no-ops cleanly when env isn't set).
  - **Market resolution.** New `lib/db/market-resolution.ts`. `resolveMarket(marketId, outcome)`: verifies the caller is the market creator, flips `markets.status = 'resolved'` + sets `outcome`, credits **100⚡ per share** to every winning position (via service-role profile updates), inserts synthetic `sell` trades at 100 for ledger accuracy, deletes settled positions, and fans out `market_resolved` notifications to all holders. Idempotent against re-resolution. New `app/market/[id]/actions.ts` exposes `resolveMarketAction`. New `components/market/resolve-market-dialog.tsx`: YES/NO selector with a destructive-confirmation step, sonner loading/success/error toasts, `useTransition`-driven pending UI. The market detail page now does a parallel Supabase lookup and passes `canResolve` (creator + open status) + `dbStatus` / `dbOutcome` to the client, which renders a "Resolve" button in the hero for the creator and a "Resolved YES/NO" trophy badge once settled.
  - **Bookmarks.** New `lib/db/bookmarks.ts` (`listMyBookmarks`, `toggleMarketBookmark`) returns `{posts, markets}` preserving bookmark order. `/saved` page is now `dynamic = "force-dynamic"` and pulls real bookmarks for signed-in users, mapping rows to the `UserPost` shape so the same `PostCard` renders. `components/saved-view.tsx` accepts optional `initialPosts` / `initialMarkets` props; when present they win, otherwise falls back to the local Zustand stores. Tabs sync `#posts` / `#markets` in the URL for shareable deep-links.
  - **Onboarding.** New `app/onboarding/actions.ts` `completeOnboardingAction({categories})` merges per-category 0.7 affinity into `profiles.affinity` and sets `onboarded = true`, with `revalidatePath('/')` so the feed picks up the new bias. The 4-step client flow now toasts "Saving your preferences…" → "Welcome to Zap ⚡" via `useTransition`, persists to Supabase when available, and still completes locally with a `(offline mode)` toast when the action errors. Step 4 button shows a "Saving…" pending state.
  - **No-backend safety.** New `hasSupabaseEnv()` guard in `lib/supabase/client.ts` plus matching short-circuits in `lib/realtime.ts` and `NotificationBell` so the prototype renders without env (the previous code threw "Your project's URL and API key are required" on `/notifications` and `/saved`).
- ✅ **Phase 10** — Prototype scaffolding deleted; dev console is clean.
  - **Types extracted.** New `lib/types.ts` holds every shared type that lived in the old mock-data file (`Category`, `User`, `Market`, `Post*`, `Trade`, `Comment`, `SeededComment`, `Notification`, `ExpertScores`, plus the `CATEGORIES` const and `CURRENT_USER_ID`). Values-free; safe for both client and server import paths.
  - **Fixtures consolidated.** New `lib/fixtures.ts` is the single home for the demo data the prototype still leans on (landing ambient feed, right-rail experts, leaderboard podium, attach-market dialog, etc.). It re-exports every type from `lib/types.ts` so the bulk import-path rewrite below could be a one-token swap.
  - **mock-data.ts deleted.** 860 lines of seeded fixture data removed. All 39 consumer files now import from `@/lib/fixtures` (values) or `@/lib/types` (types). The product surfaces (profile, feed, markets, /saved, /notifications) keep reading from Supabase via `lib/db/*` with the fixtures as a last-resort fallback.
  - **fake-socket.ts deleted.** The old polling shim was already superseded by `lib/realtime.ts` in Phase 1. `live-feed-provider.tsx` is now a documented no-op so layouts that mount it keep compiling without touching every layout call site.
  - **Zustand persist removed.** `store.ts` no longer wraps the creator in `persist(...)` and no longer imports `persist` / `createJSONStorage`. The store is in-memory only; `useHydrateZapStore` is now a no-op kept for source compatibility; `useHydrated` still exists because dozens of components mount-gate on it. This was the root cause of the long-standing client/server hydration mismatch warning — gone.
  - **Tiptap underline duplicate.** `StarterKit.configure({ underline: false })` so the standalone `@tiptap/extension-underline` doesn't clash with StarterKit v3's bundled underline node. Killed the recurring `[tiptap warn]: Duplicate extension names found: ['underline']` console spam.
  - **Nested-button hydration warning.** The topbar's Sign Out menu item was rendering `<form><button>Sign Out</button></form>` inside a Radix DropdownMenuItem (which itself is a button). Refactored to call `form.requestSubmit()` from `onSelect` so there's exactly one button in the tree.
  - **Browser verification.** Cold `.next` rebuild + smoke test on `/`, `/markets`, `/feed`, `/saved`, `/profile/you`, `/notifications`, `/leaderboard`, `/onboarding` — all `200 OK`, 14 articles render on the home feed, composer dialog opens with a working Tiptap editor, and `preview_console_logs` reports zero entries at `warn` / `error` levels.
- ✅ **Phase 11** — Quests and Ads System (initial draft, audited + fixed in Polish 1).
  - Added **Quests** page (`src/app/quests/page.tsx`) with daily tasks, progress tracking, confetti, and UI integration in topbar.
  - Created **Ad** schema (`supabase/migrations/0005_ads.sql`) with `ads` table and RLS policies restricting creation/modification to admin role.
  - Planned admin UI (`src/app/admin/ads/page.tsx`) and components `AdBanner`, `NativeAdSlot` for rendering ads in feed and sidebars.
- ✅ **Polish 1 (Phase 12)** — Hashtags, user-submitted markets, admin tooling, explainable expert scoring, and a bug sweep on the Phase-11 additions.
  - **New schema** in `supabase/migrations/0003_polish_1.sql`:
    - `hashtags(id, slug unique, uses, created_at)` + `post_hashtags(post_id, hashtag_id)` join (RLS: public read, post-author write).
    - `market_status` enum extended with `'pending'` and `'rejected'`; `markets.rejection_reason` added; `markets` SELECT policy rewritten so pending/rejected rows are only visible to the proposer or an admin; INSERT policy lets signed-in users insert their own `status='pending'` proposals.
    - `profiles.is_admin boolean default false`; idempotent `bootstrap_admin(uuid)` helper sets `is_admin=true` and `zaps=9_999_999`. Run via env `ADMIN_USER_ID` (set the GUC `app.admin_user_id` before migrating, or call `select public.bootstrap_admin('<uuid>')` after).
    - `expert_scores` gains `accuracy real` and `expertise real` columns. New `recompute_expert_scores(category_id)` walks every user with resolved positions in the category, recomputes Brier / calibration / accuracy / expertise / percentile, and is idempotent. `expert_factor_for` now reads `expertise` (composite) instead of raw `percentile`.
    - `compute_exposure_score` rewritten with `hashtag_affinity_for(user, post)` factor: `0.40 expert + 0.25 engagement + 0.15 freshness + 0.05 category_affinity + 0.05 hashtag_affinity + 0.10 boost − cooldown` (still sums to 1.0).
    - `profiles.affinity` jsonb migrated to `{categories: {…}, hashtags: {…}, last_filter}` shape. Legacy flat rows are coerced in-place. `category_affinity_for` reads either shape.
    - `decay_affinity_weekly()` pulls every numeric affinity score 5% toward 0.3. Call from `pg_cron` or a server action once per week.
  - **Hashtag pipeline** (isomorphic): `src/lib/hashtags.ts` (`extractHashtags`, `linkifyHashtags`, `detectOpenHashtagPartial`, `slugifyHashtag`). Server: `src/lib/db/hashtags.ts` (`syncPostHashtags`, `searchHashtags`, `listPostsByHashtag`, `listTopHashtags`). Wired into `createPost` so every post create/update syncs tags + bumps `uses`. New `/api/hashtags?q=` endpoint, new `/tag/[slug]` route, new `<HashtagPopover>` autocomplete (mirrors mention popover, supports "Create #foo" affordance), new `<HashtagChips>` strip rendered under every PostCard body. PostCard body HTML is run through `linkifyHashtags` so inline `#foo` tokens become real `/tag/foo` links. Exposure-explain popover gained a "Hashtag affinity" row.
  - **User-submitted markets**: new `/api/categories` lookup, `src/components/market/propose-market-dialog.tsx` form (question / description / category chips / resolution date / source) with full validation. Topbar "Propose" button (desktop) + drawer link (mobile). Server action `proposeMarketAction` (in `src/app/catalog/actions.ts`) writes `status='pending'` and fans out a `market_proposed` notification to every admin. Pending and rejected proposals show up on `/profile/[u]` under a new **Proposed** tab (`<ProposedMarketCard>`) with status badge and rejection reason when applicable.
  - **Admin tooling**: `src/lib/db/admin.ts` exposes `requireAdminContext()` + `listMarketsForAdmin(status)`. `/admin` redirects to `/admin/markets`; `/admin/markets` is server-gated (`redirect('/')` on non-admin) with three tabs (Pending / Open / Resolved). Approve → status='open' + notify proposer. Reject → status='rejected' with confirm-modal-captured reason + notify proposer. Resolve → YES / NO / CANCELLED via a single `resolveMarket(marketId, outcome)`. Cancelled refunds each holder `shares × avg_price`; YES/NO pays winners `shares × 100`. Synthetic `sell` trades recorded at the settlement price. Every holder receives a per-recipient `market_resolved` notification including their P&L number. Resolution then calls `recompute_expert_scores(category_id)`.
  - **Expert scoring UI**: new `src/components/profile/real-expert-scores.tsx` renders the live Brier / Calibration / Accuracy / Expertise / Percentile from Supabase per category, each with an info-icon Popover that quotes the metric definition verbatim from the spec. Volume-thresholded ("new · default") chip when `<5` resolved markets.
  - **Bugs fixed in the Phase 11 additions**:
    - `0004_quests_and_monster_algo.sql` — quest seed rows were typed as `'q-daily-view-1'` strings against a `uuid` PK; rewritten with `gen_random_uuid()` and idempotency keyed on `title` (now unique). `claim_quest_reward` was inserting `title`/`body` columns that don't exist on `notifications`; rewritten to use `type='quest_claimed'` + jsonb `payload`. Monster-algo weights summed to 1.05; rebalanced to 1.00. `engagement_velocity_for` divisor clamped so fresh posts don't spike to 10× engagement.
    - `0005_ads.sql` — switched from `uuid_generate_v4()` to `gen_random_uuid()`, made table/index/policy creation idempotent (`if not exists` + `drop policy if exists`), and switched the admin check from `auth.users.app_metadata->>'role'` (never populated) to canonical `profiles.is_admin`.
    - `/api/quests/[questId]/claim/route.ts` — original constructed a module-scope service-role client; the SQL function does `auth.uid()` which is NULL under service-role, so every claim threw "Not authenticated". Rewritten to use the cookie-bound server client. Next 16 `params` is awaited. UUID validation + friendly 400/401 responses.
    - `/quests/page.tsx` — was untyped JSX with hardcoded mock progress and a Claim button that only fired confetti. Replaced with a server-rendered page that fetches `quests` + `user_quest_progress` + `profiles.zaps`, plus a client component that groups by daily/weekly/lifetime, animates progress bars, calls the real claim API, and refreshes RSC data on success.
    - `feed-stream.tsx` — dropped the deprecated `react-virtual` package and the inner-scroll container (`calc(100vh - 200px)` was clipping the composer). Removed the racy `isScrolling` + buffer flush (two effects both reset `setSortedPosts`). Hashtag affinity folded into the prototype ranking so it mirrors the Supabase formula. Also dropped unused `blurhash` and `web-vitals` from `package.json`.
    - `topbar.tsx` — fixed broken indentation of `navItems`, added the Quests link properly, plus a desktop "Propose" button + drawer entry, plus the propose-market dialog mount.
